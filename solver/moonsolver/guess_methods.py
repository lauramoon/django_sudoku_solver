...
def guess_recursive(p, start_index):
    ...
    for i in range(start_index, 81):
        ...
        for possible in possibles:
            ...
            # check if multiple solutions found
            if len(p.valid_completion_list) >= 2:
                p.multiple_solution = True
                # Add error string if this is the first level with two solutions
                if p.error_description == '':
                    p.error_description = f"different values possible in row {p.box_map[i].row +1}, " \
                                      f"column {p.box_map[i].col + 1}"
                break
